...
var callMonitor					= new adapter({
	"name": "Callmonitor",
	"loglevel": 1,
	"description": "Verbindet sich mit der Fritzbox und benachrichtigt über neue Telefonanrufe.",
	"settingsFile": "callmonitor.json"
});
...
var monitor = new CallMonitor(callMonitor.settings.ip, callMonitor.settings.callmonitor);

monitor.on('inbound', function (call) {
	callMonitor.log.debug("klingelt:" + call.caller);
	callMonitor.setVariable("festnetz.lastCaller", call.caller);
	callMonitor.setVariable("festnetz.status", "klingelt");
});

monitor.on('outbound', function (call) {
	callMonitor.log.debug("ausgehend");
	callMonitor.setVariable("festnetz.status", "ausgehend");
});

monitor.on('connected', function (call) {
	callMonitor.log.debug("eingehend");
	callMonitor.setVariable("festnetz.status", "angenommen");
});

monitor.on('disconnected', function (call) {
	callMonitor.log.debug("aufgelegt");
	callMonitor.setVariable("festnetz.status", "aufgelegt");
});