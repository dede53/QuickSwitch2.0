...
module.exports = function(app, db){
				...
				groupFunctions.getGroup(id, req, res, function(group){
					if(group == "404"){
						...
						groupFunctions.switchGroup(app, group[0], status, req, res, function(data){
							if(data == 200){
								console.log('Successful: Switch Group with id: ' + id + " to " + status);
								res.json(200);
							}else{
								res.json(data);
							}
			...
			case "room":
				roomFunctions.getRoom(id, req, res, function(room){
					roomFunctions.switchRoom(room, status, req, res, function(data){
						if(data == 200){
							console.log('Successful: Switch Room with id: ' + id + " to " + status);
							res.json(200);
						}else{
							res.json(data);
						}
	...
	app.get('/timer/:id', function (req, res) {
		var id = req.params.id;
		timerFunctions.getTimer(id, req, res, function(data){
			res.json(data);
		});
	});
	...
	app.get('/sensor/:id/:date', function (req, res) {
		temperatureFunctions.getSensorvalues(req.data.date, req, res, function(data){
			res.send(data);
		});
	});
