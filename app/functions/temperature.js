...
module.exports = {
	...
	getSensorvalues: function (interval, req, res, callback) {
			...
			if(err){
				...
				async.each(sensor,
					function(sensor, callback){
						switch(interval){
							case "latest":
								var query = "SELECT * FROM sensor_data ORDER BY id DESC LIMIT 1;";
								break;
							default:
								var query = "SELECT nodeid, time, temp / 100 as temp, ROUND(time / 1000) as timestamp FROM sensor_data WHERE nodeid = '" + sensor.nodeID + "' AND (time/1000) <= UNIX_TIMESTAMP() AND (time/1000) >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL " + interval + " hour)) GROUP BY (ROUND(timestamp/3600)*3600) ORDER BY time ASC;";
								break;
						}
