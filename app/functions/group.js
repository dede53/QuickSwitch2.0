...
module.exports = {
	switchGroup: function(app, group, status, req, res, callback){	
		...
		var query = "SELECT deviceid, status, devices.name, protocol, buttonLabelOff, buttonLabelOn, switchserver, CodeOn, CodeOff,devices.roomid, rooms.name AS Raum FROM devices, rooms WHERE devices.roomid = rooms.id AND ("+ string +");"
